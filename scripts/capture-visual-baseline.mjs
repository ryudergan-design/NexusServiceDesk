...
const DATE_TAG = "2026-03-15-baseline";
...
const SCREENS = [
  ...
  { key: "chamados-detalhe", url: "/dashboard/tickets/102", auth: true },
...
async function captureDevice(deviceName, options, folderName) {
  ...
  try {
    ...
    for (const screen of SCREENS) {
      ...
      const targetDir = path.join(ROOT_DIR, folderName, screen.key, DATE_TAG);
      await ensureDir(targetDir);
      await page.screenshot({
        path: path.join(targetDir, "atual.png"),
        fullPage: true,
        type: "png",
      });
    }

    console.log(`Capturas ${deviceName} geradas.`);
  } finally {
    await context.close();
    await browser.close();
  }
