...
import { AIAssignmentSequence } from "@/components/ui/ai-assignment-sequence"
...
export function RobotAssignment({
  ...
  const [assigningAgentName, setAssigningAgentName] = useState<string | null>(null)
  ...
  const handleAssign = async (agentId: string, agentName: string) => {
    ...
    try {
      ...
      setLoading(false)
      window.setTimeout(() => setAssigningAgentName(null), 450)
    }
  }
  ...
  const isAIActive = agents.some((agent) => agent.id === currentAssigneeId)

  return (
    <>
      <AIAssignmentSequence active={Boolean(assigningAgentName)} agentName={assigningAgentName || undefined} />
      ...
      <Popover open={open} onOpenChange={setOpen}>
        ...
        <PopoverContent
          className="w-64 bg-slate-900 border-white/10 p-2 shadow-2xl rounded-2xl"
          sideOffset={10}
        >
